...
export function Mark({ size = 22, className = "" }: { size?: number | string; className?: string }) {
...
export function Logo({ size = "md", glint = false }: { size?: "sm" | "md" | "xl"; glint?: boolean }) {
  const px = size === "xl" ? "text-[clamp(3.4rem,10.5vw,7.6rem)]" : size === "sm" ? "text-[1.05rem]" : "text-[1.25rem]";
