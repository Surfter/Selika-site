...
function Tile({ id, dense }: { id: string; dense?: boolean }) {
  ...
  return (
    <motion.div layout className={`glass flex min-h-0 flex-col justify-center rounded-2xl ${dense ? "px-2.5 py-2" : "px-3 py-2.5"} ${wide ? "col-span-2" : ""}`}
  ...
  return (
    <motion.div className={`absolute inset-0 z-10 flex flex-col p-3 pt-9 ${dense ? "dense gap-1.5" : "gap-2"}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      {on.length === 0 ? (
        ...
        <motion.div layout className={`grid min-h-0 flex-1 auto-rows-fr grid-cols-2 items-stretch overflow-y-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${dense ? "gap-1.5" : "gap-2"}`}
