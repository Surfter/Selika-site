import { AnimatePresence, motion, useAnimationFrame, useMotionValue, useScroll, useTransform, type MotionValue } from "framer-motion";
...
function HeroFace() {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-5">
      ...
      <Mark size={40} />
      <Logo size="xl" glint />
      <div className="h-7 overflow-hidden text-[0.95rem] font-medium tracking-[0.22em] uppercase text-mute">
...
/* ============================================================
   ...
   ============================================================ */
function Slab({ hero, demo, heroOpacity, demoOpacity, footer, footerOpacity, footerHeight }:
  { hero: ReactNode; demo: ReactNode; heroOpacity: MotionValue<number> | number; demoOpacity: MotionValue<number> | number; footer?: ReactNode; footerOpacity?: MotionValue<number> | number; footerHeight?: MotionValue<number> | number }) {
  ...
  return (
    <div className="relative h-full w-full" style={{ transformStyle: "preserve-3d" }}>
      ...
      {/* back face */}
      <div className="absolute inset-0 rounded-[28px] glass glass-strong flex items-center justify-center" style={{ position: "absolute", transform: "rotateY(180deg) translateZ(1px)", backfaceVisibility: "hidden" }}>
        <Mark size={54} />
      </div>
      {/* front face */}
      <div className="absolute inset-0 flex flex-col rounded-[28px] glass glass-strong overflow-hidden" style={{ position: "absolute", backfaceVisibility: "hidden", background: "linear-gradient(135deg, rgba(20,24,40,0.86), rgba(12,14,24,0.92))", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.22), 0 0 0 1px rgba(142,197,255,0.25), 0 0 80px -12px rgba(77,141,255,0.55), 0 40px 90px -30px rgba(0,0,0,0.9)" }}>
        ...
        {footer && <motion.div className="flex-none overflow-hidden" style={{ opacity: footerOpacity ?? 1, height: footerHeight ?? 108 }}>{footer}</motion.div>}
      </div>
    </div>
  );
}
        ...
        <AnimatePresence mode="wait">
          {s.mode === "product" ? (
            <motion.div key="subs" className="flex flex-wrap justify-center gap-2" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.25 }}>
              {([["env", "Environment"], ["hud", "Reflection HUD"], ["tx", "Transformation"]] as [Sub, string][]).map(([v, l]) => (
...
function HeroCopy({ style }: { style?: React.CSSProperties | any }) {
  return (
    <motion.div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center px-5 text-center" style={style}>
      <motion.div className="glass mb-5 inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-[0.7rem] font-semibold uppercase tracking-[0.18em] text-mute"
        initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease, delay: 0.3 }}>
        <span className="h-1.5 w-1.5 rounded-full bg-sky shadow-[0_0_10px_#8EC5FF]" /> Prototype &amp; validation stage
      </motion.div>
...
function StageDesktop({ s }: { s: DemoState }) {
  ...
  const footerHeight = useTransform(q, [0.4, 0.85], [0, 108]);
  ...
  return (
    <div ref={wrapRef} id="top" className="relative" style={{ height: "300vh" }}>
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        {/* ---- hero layer ---- */}
        <motion.div className="absolute inset-0 flex flex-col items-center pt-[11vh]" style={{ pointerEvents: heroPE }}>
          <HeroCopy style={{ opacity: heroCopyOpacity, y: heroCopyY }} />
          {/* where the mirror lives while the hero is up */}
          <div ref={heroSlot} aria-hidden className="mt-7" style={{ width: "min(25vh, 16rem)", aspectRatio: "4 / 5.9" }} />
          <motion.div className="absolute bottom-7 flex flex-col items-center gap-1 text-[0.66rem] uppercase tracking-[0.2em] text-dim" style={{ opacity: hintOpacity }}>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center pt-24 pb-6">
          ...
          <div className="wrap grid grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)] items-start gap-12">
            <div className="flex justify-center">
              <div ref={demoSlot} aria-hidden style={{ width: "min(46vh, 25rem)", aspectRatio: "4 / 5.9" }} />
            </div>
            <motion.div className="max-h-[calc(100vh-13rem)] overflow-y-auto pr-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" style={{ opacity: controlsOpacity, x: controlsX, pointerEvents: controlsPE, maskImage: "linear-gradient(#000 88%, transparent)", WebkitMaskImage: "linear-gradient(#000 88%, transparent)" }}><DemoControls s={s} /></motion.div>
          </div>
        ...
        <motion.div className="absolute z-20" style={{ left, top, width, height, scale, rotateY, rotateX, transformPerspective: 1500, transformStyle: "preserve-3d", willChange: "transform" }}>
          <Slab hero={<HeroFace />} demo={<DemoFace s={s} />} heroOpacity={heroFace} demoOpacity={demoFace} footer={<DemoFooter s={s} />} footerOpacity={footerOpacity} footerHeight={footerHeight} />
        </motion.div>
      </div>
    </div>
  );
  ...
  return (
    <>
      <section id="top" className="relative flex min-h-[100svh] flex-col items-center overflow-hidden pt-28 pb-16">
        ...
        <Shards count={5} />
        <HeroCopy />
        <motion.div className="relative z-10 mt-12" style={{ width: "min(58vw, 15rem)", aspectRatio: "4 / 5.9", rotateY, rotateX: 7, transformPerspective: 1200, transformStyle: "preserve-3d" }}>
          <Slab hero={<HeroFace />} demo={null} heroOpacity={1} demoOpacity={0} />
        </motion.div>
      </section>
      <section id="demo" className="relative pt-16 pb-20">
        <div className="wrap flex flex-col items-center gap-6">
          ...
          <div style={{ width: "min(88vw, 24rem)", aspectRatio: "4 / 5.9" }}>
            <Slab hero={null} demo={<DemoFace s={s} />} heroOpacity={0} demoOpacity={1} footer={<DemoFooter s={s} />} />
          </div>
